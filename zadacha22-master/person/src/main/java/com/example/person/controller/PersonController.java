...
public class PersonController {
    ...
    public ResponseEntity<Weather> getWeatherForPersonLocation(@PathVariable int id) {
        ...
        if (personOptional.isPresent()) {
            ...
            Geodata geodata = restTemplate.getForObject("http://localhost:8081/location?location=" + person.getLocation(), Geodata.class);
            if (geodata != null) {
                String weatherServiceUrl = "http://localhost:8082/weather/lat=" + geodata.getLat() + "&lon=" + geodata.getLon();
